---
name: make
description: Implement the current bbFlow brief. Use when the user runs /make or asks to build the briefed ticket.
...
1. Read root `AGENTS.md` if present and `.cursor/bbflow/CONTEXT.md`. If CONTEXT is missing, suggest `/context` once, but do not block.
2. Read `.cursor/bbflow/BRIEF.md`. If it is missing, stop and suggest `/brief`.
...
8. Update the board: **Phase** `proving` if ready (Next `/prove`), otherwise stay `making` and name the blocker.
...
- Do not claim done before `/prove`.
